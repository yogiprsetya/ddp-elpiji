...
export default {
	...
	methods : {
        login(){
            ...
            axios({
                method: 'post',
                url: process.env.API_HOST + '/api/xyz-login.php',
                data: formData,
                config: { headers: {'Content-Type': 'multipart/form-data' }}
            })
            .then(response => {
                localStorage.tknxyz = response.data[0].id_user
                if (response.data[0].id_user) {
                    window.location = "#/xyz-dashboard"
                }
            })
            .catch(response => {
        ...
        checkToken() {
			if (localStorage.getItem('tknxyz') != null) {
				window.location.assign('#/xyz-dashboard')
			}
		}
    },
    mounted() {
